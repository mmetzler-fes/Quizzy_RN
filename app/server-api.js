...
module.exports = function setupApi(app, userDataPath) {
	...
	console.log('Database path:', dbPath); // Print path for debugging
	...
	// Vokabeln (similar structure logic...)
	...
	app.get('/api/vokabelnSearch', (req, res) => {
		const db = readDB();
		const lower = (req.query.q || '').toLowerCase();
		res.json(db.vokabeln.filter(i => i.name.toLowerCase().includes(lower) || i.vokabel.toLowerCase().includes(lower)));
	});
	...
	console.log('Quizzy API Routes Initialized');
};
